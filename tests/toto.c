...
gcc -g -O2 -Wall -Wundef -Wstrict-prototypes -Wno-trigraphs -fno-strict-aliasing -fno-common -Werror-implicit-function-declaration -DCONFIG_LIBNL30 -I/usr/include/libnl3 -c toto.c -o toto.o

cc toto.o -g -lnl-route-3 -lnl-genl-3 -lnl-3 -o exe_toto
...
int main(int argc, char **argv) {
  ...
  struct tpacket_req3 packet_req = {
  ...
  struct tpacket_req3 packet_req_rx[nbraws], packet_req_tx[nbraws];
  uint8_t *mmap_rx[nbraws];
  for (uint8_t i = 0; i < nbraws; i++) {
    ...
    memcpy(&packet_req_rx[i], &packet_req, sizeof(struct tpacket_req3));
/*
    if (-1 == setsockopt (rawfds[i], SOL_PACKET, PACKET_RX_RING, &packet_req_rx[i], sizeof(struct tpacket_req3))) exit(-1); // FIRST
*/
    if (MAP_FAILED == (mmap_rx[i] = mmap(0, rawmmap[i].map_size * 2, PROT_READ|PROT_WRITE, MAP_SHARED, rawfds[i], 0))) exit(-1);
