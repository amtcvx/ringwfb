/*
gcc packet_mmap_nothread.c -o packet_mmap_nothread
*/
...
int main(int argc, char **argv) {
  ...
  uint8_t *const map = mmap( NULL, req.tp_block_size * req.tp_block_nr, PROT_READ|PROT_WRITE, MAP_SHARED | MAP_LOCKED, rxsock, 0 );
  if( map == MAP_FAILED ) exit(-1);
...
/*
	...
	if (setsockopt(fd_socket, SOL_PACKET, PACKET_QDISC_BYPASS, &one, sizeof(one)) < 0 ) exit(-1);
	...
	for (i=0; i<s_packet_req.tp_block_nr; i++ ) {
	  ...
	  for( j=0; j<c_packet_sz; j++ )  pkt_ptr[j] = j;
